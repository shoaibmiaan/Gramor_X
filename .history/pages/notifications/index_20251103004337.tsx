...
function formatTimestamp(iso: string): string {
  ...
  return dt.toRelative({ style: 'long' }) ?? dt.toLocaleString(DateTime.DATETIME_MED);
}
...
export const getServerSideProps: GetServerSideProps<NotificationsPageProps> = async (ctx) => {
  ...
  // --- FIXED ZOD-SAFE MAPPING ---
  const notifications = (data ?? []).map((row) => ({
    id: row.id,
    message: row.message ?? 'Notification',
    url: row.url && row.url.trim() !== '' ? row.url : null, // ensure URL is null or valid
    ...
    createdAt:
      row.created_at instanceof Date
        ...
        : new Date().toISOString(), // ensure valid ISO string
  }));
