...
/**
 * NOTE:
 * - Small reusable UI primitives are defined below (ActionButton, OverviewCard, StatItem, AttemptCard).
 * - These are intentionally minimal and thin: they centralize classnames & component usage so the rest
 *   of the portal can import and re-use => maintain consistent UI.
 *
 * Replace or extend these wrappers with centralized tokens/design-system changes when you want a global UI update.
 */

/* --------------------------- Small UI primitives --------------------------- */

type ActionButtonProps = React.ComponentProps<typeof Button> & { to?: string };
const ActionButton = ({ to, children, ...rest }: ActionButtonProps) =>
  to ? (
    <Button {...rest} href={to}>
      {children}
    </Button>
...
/** Card variant for overview panels — central place to change padding / surface look */
const OverviewCard: React.FC<React.PropsWithChildren<{ className?: string }>> = ({ children, className = '' }) => {
  return <Card className={`card-surface p-6 sm:p-8 ${className}`}>{children}</Card>;
};
    ...
    <WritingLayout plan={__plan} current="overview">
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] mt-6">
        <OverviewCard>
          ...
          <div className="mt-4 rounded-2xl border border-dashed border-border/50 bg-muted/40 p-4 text-sm text-muted-foreground">
            {/* central place for plan-dependent copy */}
        ...
        <OverviewCard>
          ...
          {microPromptState.alreadySentToday && <p className="mt-2 text-xs text-muted-foreground">Tip already delivered today — check back tomorrow for a fresh cue.</p>}
        </OverviewCard>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2 mt-6">
        <OverviewCard>
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge variant="soft" tone="default" size="sm">{stats.activeDrafts} active</Badge>
          </div>
        ...
        <OverviewCard>
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge variant="soft" tone="default" size="sm">Last {Math.min(stats.recentAttempts.length, 6)}</Badge>
          </div>
      ...
      <OverviewCard className="mt-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Badge variant="soft" tone="info" size="sm">Curated resources</Badge>
        </div>
