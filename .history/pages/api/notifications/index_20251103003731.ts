...
const BASE_URL = 'https://gramorx.com'; // <-- replace with your actual domain
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    // fallback welcome notification if list is empty
    if (list.length === 0) {
      const welcome = {
        id: 'welcome',
    ...
    return res.status(200).json({
      notifications: list.map(({ createdAt, ...rest }) => ({ ...rest, created_at: createdAt, url: rest.url })),
