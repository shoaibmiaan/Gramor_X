...
import { CreateNotificationSchema, NotificationNudgeSchema } from '@/lib/schemas/notifications';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (error || !data) return res.status(500).json({ error: error?.message || 'Failed to create notification' });
