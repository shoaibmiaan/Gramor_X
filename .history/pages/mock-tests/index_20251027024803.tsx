...
// If you have a tracker util, great. Otherwise safely no-op this import.
import { track } from '@/lib/analytics/trackor'; // keep type-safe if available
import { mockTests } from '@/data/mock-tests';

export default function MockTestsIndex() {
  const handleClick = (slug: string, skill: string) => {
    // Non-blocking client analytics (no await)
    try {
          ...
          {mockTests.length === 0 ? (
            <Card className="card-surface p-8 rounded-ds-2xl">
              <div className="flex flex-col gap-3">
                ...
                {/* Use Link for internal nav per team rule */}
                <div className="mt-2 flex gap-3">
                  {/* If Button supports `asChild`, prefer that. */}
              ...
              {mockTests.map((m) => {
                ...
                return (
                  <li key={m.slug} role="listitem">
                    {/* Prefer semantic article; ensure DS Card forwards the element type or wrap inside */}
                    <Card as="article" className="card-surface p-6 rounded-ds-2xl" aria-labelledby={headingId}>
                      ...
                      <div className="mt-6">
                        {/* Internal nav must use Link. Use Button `asChild` (Radix Slot pattern). */}
                        <Button
                          variant="primary"
