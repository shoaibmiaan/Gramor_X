...
const PinInput = React.forwardRef<HTMLDivElement, PinInputProps>(
  ({ length = 6, value, onChange, disabled, invalid }, ref) => {
    ...
    // keep local refs length in sync
    React.useEffect(() => {
    ...
    return (
      <div ref={ref} className="pr-space-y-2">
        <div
          className={[
            'pr-grid pr-grid-cols-6 pr-gap-2 sm:pr-gap-3',
            disabled ? 'pr-opacity-70 pr-pointer-events-none' : '',
          ].join(' ')}
        >
          {digits.map((d, i) => (
            <input
              ...
              className={[
                'pr-w-11 pr-h-12 sm:pr-w-12 sm:pr-h-14 pr-rounded-xl pr-text-center pr-text-h4 pr-font-semibold',
...
export default function PremiumPinPage() {
  ...
  async function submitPin(e?: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        await router.replace(nextUrl); // API sets cookie; middleware will allow through
        return;
      }
