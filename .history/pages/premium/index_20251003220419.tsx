...
const prettyDate = (isoDay: string | null) => {
  if (!isoDay) return '—';
  try {
...
function usePremiumStatus(token: string | null) {
  ...
  return { data, loading, refresh: () => setData((s) => ({ ...s! })) };
}
...
function useStreak(token: string | null) {
  ...
  const bumpToday = React.useCallback(async () => {
    try {
      const res = await fetch('/api/streak', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({}), // core “completion” path (see API)
      });
      if (res.ok) {
        const json: StreakData = await res.json();
        setData(json);
...
export default function PremiumHome() {
  ...
  const { data: streak, loading: streakLoading, bumpToday } = useStreak(token);
  ...
  return (
    <>
      ...
      <main className="pr-relative pr-mx-auto pr-max-w-[1200px] pr-p-6 pr-space-y-8">
        {/* header */}
        ...
        <section className="pr-grid md:pr-grid-cols-3 pr-gap-6">
          <PrCard className="pr-p-6 pr-flex pr-flex-col pr-justify-between">
              ...
              <div>
                ...
                <p className="pr-muted pr-text-small pr-mt-1">
                  {streak?.last_activity_date
                    ? `Last: ${prettyDate(streak.last_activity_date)}`
                    : 'No activity yet'}
                </p>
              </div>
            ...
            <div className="pr-mt-6 pr-flex pr-gap-3 pr-flex-wrap">
              <PrButton onClick={bumpToday}>Log Today</PrButton>
              <PrButton
                variant="secondary"
                onClick={() => router.push('/premium/listening/sample-test')}
              >
                Take a Sample
              </PrButton>
          ...
          <PrCard className="pr-p-6 pr-flex pr-flex-col">
            ...
            <div className="pr-mt-4 pr-flex pr-gap-3 pr-flex-wrap">
        ...
        <section className="pr-space-y-3">
          ...
          <div className="pr-grid md:pr-grid-cols-4 pr-gap-6">
            <PrCard className="pr-p-6 pr-flex pr-flex-col pr-gap-3">
              ...
              <div className="pr-flex pr-gap-2 pr-mt-auto pr-items-center">
        ...
        <section className="pr-grid md:pr-grid-cols-2 pr-gap-6">
          <PrCard className="pr-p-6 pr-flex pr-flex-col pr-gap-3">
            <div className="pr-flex pr-items-center pr-gap-2">
              <Sparkles className="pr-w-5 pr-h-5" aria-hidden />
