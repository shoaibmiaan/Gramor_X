...
type Props = {
  userId: string;
  plan: string | null; // 'free' | 'starter' | 'booster' | 'premium' | 'master' | null
};
...
export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  ...
  if (meta.mfa_enabled && !meta.mfa_verified) {
    return {
      redirect: {
  ...
  if (!hasPremium) {
    return {
      redirect: {
        destination: `/pricing?reason=premium_required&next=${encodeURIComponent(
          ctx.resolvedUrl || '/premium',
        )}`,
        permanent: false,
      },
    };
