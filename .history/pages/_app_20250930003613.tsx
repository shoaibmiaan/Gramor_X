...
function InnerApp({ Component, pageProps }: AppProps) {
  ...
  // Keep backend session cookie in sync AND redirect correctly on auth changes.
  // This fixes "stuck on login until refresh" and "premium bouncing to /login".
  useEffect(() => {
    if (IS_CI) return;
    ...
    const syncSession = async (event: AuthChangeEvent, sessionNow: Session | null) => {
      ...
      try {
        ...
        // silent: server cookie sync is best-effort
      }
    };

    // On first load: sync cookie and if already signed-in on an auth page, go to next/home
    (async () => {
      const {
        data: { session },
    ...
    const {
      ...
      (async () => {
        // 1) Make server cookie authoritative before we navigate
