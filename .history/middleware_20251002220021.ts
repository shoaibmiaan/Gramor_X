...
export async function middleware(req: NextRequest) {
  ...
  const isPremiumPinPage =
    pathname === '/premium/pin' || pathname === '/premium-pin'; // allow both spellings

  if (isPremiumSection) {
    // Always allow the PIN entry page
    if (isPremiumPinPage) return res;

    // Check cookie set by /api/premium/verify-pin
