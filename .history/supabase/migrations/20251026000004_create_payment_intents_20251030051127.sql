...
create unique index if not exists payment_intents_gateway_session_key on public.payment_intents (gateway_session_id);

alter table public.payment_intents enable row level security;

create policy if not exists "payment_intents_self_select"
...
create index if not exists payment_intent_events_intent_idx on public.payment_intent_events (intent_id, created_at desc);
create index if not exists payment_intent_events_user_idx on public.payment_intent_events (user_id, created_at desc);

alter table public.payment_intent_events enable row level security;

create policy if not exists "payment_intent_events_self"
