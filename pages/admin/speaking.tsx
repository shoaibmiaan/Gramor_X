...
const AdminSpeakingPrompts: React.FC = () => {
  ...
  const fetchPrompts = useCallback(async () => {
    ...
    try {
      ...
      toastError(err?.message || 'Failed to load prompts');
      setPrompts([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toastError(err?.message || 'Failed to save prompt');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Container className="space-y-10">
        <div className="space-y-2">
          ...
          <p className="text-grayish max-w-2xl">
            Create new IELTS speaking prompts for teachers to use during lessons. Prompts are saved to
            Supabase and listed below for quick reference.
          </p>
        </div>
