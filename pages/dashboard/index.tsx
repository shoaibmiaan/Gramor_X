...
const DashboardPage: NextPage = () => {
  ...
  const weeklyImprovement = aggregate?.progress.activeStreak
    ? `+${Math.min(aggregate.streakDays, 20)}%`
    : '+0%';
  const nextAction =
    recommendations[0] ??
      ...
      <section className="space-y-6">
        ...
        <TierGuard tier={entitlement.tier} feature="realtimeDashboard">
          <SmartRecommendations
            items={recommendations.length ? recommendations : ['No active recommendations yet.']}
          />
        </TierGuard>
      </section>
      <AICommandCenter />
