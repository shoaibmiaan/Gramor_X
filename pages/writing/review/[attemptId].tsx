...
const buildDrillChecklist = (attempt: ReviewProps['attempt']): DrillChecklistItem[] => {
  const items: DrillChecklistItem[] = [];
  const seen = new Set<string>();

  const addItem = (id: string, label: string, hint?: string) => {
    if (seen.has(id)) return;
    seen.add(id);
    items.push({ id, label, hint, completed: false });
  };
...
const ReviewPage = ({ attempt, reviews }: ReviewProps) => {
  ...
  const drillItems = useMemo(() => buildDrillChecklist(attempt), [attempt]);
