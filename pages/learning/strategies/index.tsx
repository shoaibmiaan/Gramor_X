...
export default function Strategies() {
  ...
  const toggleSave = async (tipId: string) => {
    if (!userId) {
      // ✅ FIXED: Use object form to let Next.js encode the query value
      router.push({
        pathname: '/login',
        ...
        {!loading && err && (
          <div className="mt-10">
            <Alert variant="warning" title="Couldn’t load tips">
              {err} <br />
              {/* ✅ FIXED: Use pathname + query instead of router.asPath */}
              <Button
                variant="secondary"
                className="mt-3"
                onClick={() => router.replace(router.pathname, router.query, { shallow: true })}
              >
                Retry
              </Button>
            </Alert>
