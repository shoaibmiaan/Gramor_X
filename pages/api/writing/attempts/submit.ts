...
async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  const logger = createRequestLogger('api/writing/attempts/submit', { requestId });
  const startedAt = Date.now();

  if (!(await rateLimit(req, res))) {
    logger.warn('rate limit exceeded on submit', { requestId });
    return;
  }
  ...
  if (userError || !user) {
    logger.warn('unauthorised submit attempt', { requestId });
  ...
  if (loadError) {
    logger.error('failed to load attempt before submit', { error: loadError.message, attemptId, userId: user.id, requestId });
    return res.status(500).json({ error: loadError.message });
  }

  if (!attemptRow) {
    logger.warn('attempt not found for submit', { attemptId, userId: user.id, requestId });
    return res.status(404).json({ error: 'Attempt not found' });
  }

  if (attemptRow.user_id !== user.id) {
    logger.warn('submit forbidden', { attemptId, userId: user.id, requestId });
  ...
  if (error) {
    logger.error('failed to submit attempt', { error: error.message, attemptId, userId: user.id, requestId });
    return res.status(500).json({ error: error.message });
  }

  // TODO: enqueue scoring job via background worker
  logger.info('attempt submitted', { attemptId, userId: user.id, latencyMs: Date.now() - startedAt });
